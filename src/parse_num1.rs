...
parse_u32 是函数名称，它接收一个 input  参数，nom::IResult 是 nom 中常用的结果返回类型。可以通过[文档](https://docs.rs/nom/7.0.0/nom/type.IResult.html)查看其声明和注释：
...
它的类型由输入、输出的类型和错误类型而定，在返回 Ok 时，它包含了输入的剩余部分以及解析结果；在返回 Err 时，它包含了 `nom::Err` 类型实例。

基于 nom 的解析器时，大都是自下而上构建的，先编写最小的解析单元，然后使用组合子将它们组合到更复杂的解析器中。
nom 中已经提供了很多的基础的[解析单元](https://docs.rs/nom/7.0.0/nom/character/complete/index.html)。利用这些解析单元，我们可以做两种选择：
* 1.解析特定的内容
* 2.组合更上层的解析器

围绕这两点，我们可以开始尝试 —— 解析 `(12345)`
...
`(12345)` 由一个左小括号开始，紧跟着一批数字字符然后是右小括号结束。也就是我们可以将其拆分为：
* `(`
* `12345`
* `)`
...
use nom::character::complete::{alphanumeric0, char, digit0};
use nom::sequence::delimited;
use nom::{bytes::complete::take, character::is_digit, number::complete::be_u32, IResult};

fn parse_u32(input: &[u8]) -> IResult<&[u8], &[u8]> {
    delimited(char('('), digit0, char(')'))(input)
}
